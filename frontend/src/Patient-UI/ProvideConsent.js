...
export default function ProvideConsent(props) {
    ...
    const handleProvideConsent = async()=>{
        ...
        try {
            ...
            let putProvidedConsentsUrl = "http://localhost:4000/api/consent/" + getDetailsResp.data.id + "/" + idOfSelectedConsent;
            const response = await axios.put(putProvidedConsentsUrl, null, {
                headers: {
                    Authorization: `Bearer ${props.token}` // Assuming props.token contains the JWT token
                },
                params: {
            ...
            if (response.status === 200)
                console.log("Provided Consent sucessfully");
            else
                console.log("Provide Consent failed");
            getRequestedConsentsList()
        } catch (error) {
        ...
        {ConsentsList && (
            <div className="min-h-screen flex justify-center items-center bg-gradient-to-r from-blue-100 to-blue-500">
                <div className="bg-white p-10 rounded-lg shadow-md max-w">
                    <div className="font-bold text-7xl mb-10 text-center">Requested Consents List</div>
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 650 }} aria-label="Requested Consents List">
                            <TableHead>
                                <TableRow>
                                    <TableCell>
                                        CONSENT ID
