...
export default function PatientEHR(props) {
    ...
    const getEHRList = async () => {
        try {
            ...
            let listEHRUrl = 'http://localhost:4000/api/ehr/' + curConsultation.data.consultationRequests[0].patientId;
    ...
    const handleViewEHR = async()=>{
        ...
        try {
            ...
            let getEHRUrl = "http://localhost:4000/api/ehr/" + curConsultation.data.consultationRequests[0].patientId + "/" + idOfSelectedEHR;
            ...
            const fileURL = URL.createObjectURL(file) + '#toolbar=0';
            window.open(fileURL);
        } catch (error) {
