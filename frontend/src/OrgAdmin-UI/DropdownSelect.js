...
export default function DropdownSelect({ data, displayFieldName, selectedItem, setSelectedItem }) {
    ...
    return (
        <div className="w-full p-3 relative" ref={dropdownRef}>
            ...
            <button className="w-full text-blue-500 border-2 border-blue-500 rounded-xl py-2 px-3 focus:outline-none focus:border-blue-800 focus:text-blue-800 sm:text-sm lg:text-lg flex relative justify-between items-center" onClick={(e) => { e.preventDefault(); setIsActivated((prev) => !prev) }}>
                <div className="absolute right-10" onClick={()=>{setSelectedItem('')}}>
                    {selectedItem && <IoMdClose />}
                </div>
